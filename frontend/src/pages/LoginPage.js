// 📝 NOTE: LoginPage.js — Mobile optimised version
// 📝 NOTE: ALL auth logic, sunflower SVG, animations — UNCHANGED
// 📝 NOTE: Mobile fixes: responsive padding, smaller card on tiny screens
...
function LoginPage() {
  ...
  // 📝 NOTE: clamp padding — tighter on mobile, roomier on desktop
  const inputStyle = {
    width: '100%',
  ...
  // ── Sunflower petal helper (UNCHANGED) ──────────────────────────────────
  const renderPetals = (outerRx, outerRy, outerCy, innerRx, innerRy, innerCy) => (
    <>
      {[0,30,60,90,120,150,180,210,240,270,300,330].map(deg => (
        <ellipse key={`o${deg}`} cx="0" cy={outerCy} rx={outerRx} ry={outerRy}
          fill="#f5c842" transform={`rotate(${deg})`} opacity="0.95" />
      ))}
      {[15,45,75,105,135,165,195,225,255,285,315,345].map(deg => (
        <ellipse key={`i${deg}`} cx="0" cy={innerCy} rx={innerRx} ry={innerRy}
          fill="#e8b820" transform={`rotate(${deg})`} opacity="0.7" />
      ))}
    </>
  );
    ...
    >
      ...
      <svg className="absolute inset-0 w-full h-full" style={{ zIndex: 0 }}
        ...
        <g className="sp1">
          ...
          <ellipse cx="562" cy="460" rx="17" ry="8" fill="#1a6b1a" opacity="0.7" transform="rotate(-30 562 460)" />
          <ellipse cx="595" cy="472" rx="17" ry="8" fill="#1a6b1a" opacity="0.7" transform="rotate(30 595 472)" />
          <ellipse cx="580" cy="440" rx="13" ry="20" fill="#1e7a1e" opacity="0.6" />
        </g>
        <g className="sp2">
          <rect x="127" y="450" width="5" height="140" rx="2" fill="#155215" />
          <ellipse cx="113" cy="465" rx="17" ry="8" fill="#1a6b1a" opacity="0.7" transform="rotate(-30 113 465)" />
          <ellipse cx="145" cy="476" rx="17" ry="8" fill="#1a6b1a" opacity="0.7" transform="rotate(30 145 476)" />
          <ellipse cx="130" cy="445" rx="13" ry="20" fill="#1e7a1e" opacity="0.6" />
        </g>
        <g className="d1">
          <path d="M560 55 C560 55 544 82 544 96 C544 106 551 113 560 113 C569 113 576 106 576 96 C576 82 560 55 560 55Z" fill="#f59e0b" opacity="0.85" />
          <ellipse cx="553" cy="94" rx="4" ry="7" fill="rgba(255,255,255,0.22)" transform="rotate(-15 553 94)" />
        </g>
        ...
        <g className="d3">
          <path d="M140 75 C140 75 128 97 128 108 C128 116 133 121 140 121 C147 121 152 116 152 108 C152 97 140 75 140 75Z" fill="#f59e0b" opacity="0.7" />
          <ellipse cx="134" cy="106" rx="3" ry="6" fill="rgba(255,255,255,0.2)" transform="rotate(-15 134 106)" />
        </g>
        <g className="l1" opacity="0.5">
          <ellipse cx="500" cy="155" rx="22" ry="9" fill="#1e7a1e" transform="rotate(-35 500 155)" />
          <line x1="500" y1="146" x2="500" y2="164" stroke="#16a34a" strokeWidth="1" transform="rotate(-35 500 155)" />
        </g>
        <g className="l2" opacity="0.4">
          <ellipse cx="190" cy="195" rx="18" ry="7" fill="#1e7a1e" transform="rotate(20 190 195)" />
          <line x1="190" y1="188" x2="190" y2="202" stroke="#16a34a" strokeWidth="1" transform="rotate(20 190 195)" />
        </g>
        <g className="l3" opacity="0.35">
          <ellipse cx="640" cy="275" rx="16" ry="6" fill="#1e7a1e" transform="rotate(-15 640 275)" />
        </g>
      ...
      {/* Dark overlay */}
      <div className="absolute inset-0" style={{ zIndex: 1, background: 'rgba(8,18,9,0.60)' }} />

      {/* ── GLASS LOGIN CARD ──────────────────────────────────────────────
          📝 NOTE: w-full max-w-sm = full width on mobile, capped at 384px
          📝 NOTE: mx-3 on mobile (12px each side), mx-4 on larger screens
          📝 NOTE: padding uses clamp — smaller on tiny phones              */}
      <motion.div
        ...
        style={{
          ...
          // 📝 NOTE: clamp = 18px on tiny phone, 28px on normal phone, 38px on desktop
      ...
      >
        ...
        <div className="text-center mb-5">
          <h1 className="font-bold" style={{ color: '#f8fafc', fontSize: 'clamp(15px,4vw,18px)' }}>
            Arvi Edibles
          </h1>
        ...
        {/* Form header */}
        <p style={{ color: 'rgba(248,250,252,0.45)', fontSize: 10, fontWeight: 600, letterSpacing: '0.8px', textTransform: 'uppercase', marginBottom: 3 }}>
          Distributor Portal
        </p>
        <h2 style={{ color: '#f8fafc', fontSize: 'clamp(17px,4vw,20px)', fontWeight: 700, marginBottom: 3 }}>
          Welcome back
        </h2>
        <p style={{ color: 'rgba(248,250,252,0.45)', fontSize: 12, marginBottom: 20 }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: 14 }}>
            <label style={{ display: 'block', fontSize: 10, fontWeight: 600, color: 'rgba(248,250,252,0.45)', letterSpacing: '0.7px', textTransform: 'uppercase', marginBottom: 5 }}>
              Username
            </label>
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none"
                style={{ color: 'rgba(248,250,252,0.28)', fontSize: 14 }}>👤</span>
              <input type="text" name="username" value={formData.username} onChange={handleChange}
                placeholder="Enter your username" required style={inputStyle} onFocus={onFocus} onBlur={onBlur} />
            </div>
          </div>

          {/* Password */}
          <div style={{ marginBottom: 20 }}>
            {/* 📝 NOTE: flex-wrap so label + link wrap on very small screens */}
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 5, flexWrap: 'wrap', gap: 4 }}>
              <label style={{ fontSize: 10, fontWeight: 600, color: 'rgba(248,250,252,0.45)', letterSpacing: '0.7px', textTransform: 'uppercase' }}>
                Password
              </label>
              <span style={{ fontSize: 10, color: '#4ade80', cursor: 'pointer' }}>
            ...
            <div className="relative">
              ...
              <button type="button" onClick={() => setShowPassword(prev => !prev)}
                ...
                style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(248,250,252,0.35)' }}>
          ...
          <motion.button type="submit" disabled={loading}
            ...
            style={{ padding: '11px', borderRadius: 10, border: 'none', fontSize: 13,
        ...
        <div className="flex items-center gap-3 my-4">
          ...
          <p style={{ color: 'rgba(248,250,252,0.28)', fontSize: 10 }}>default credentials</p>
          <div className="flex-1 h-px" style={{ background: 'rgba(255,255,255,0.08)' }} />
        </div>
        ...
        <motion.div className="flex items-center gap-3"
          ...
          {/* 📝 NOTE: flex-wrap so username/password wrap on tiny screens */}
